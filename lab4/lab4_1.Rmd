...
select names that contain length in it
```{r}
...
We won't cover [regex](https://en.wikipedia.org/wiki/Regular_expression) in this class, but the following code is helpful when you know that a column contains a letter (in this case "a") followed by a subsequent string (in this case "er").  "regex" = regular expression
starts with 'a' and followed by 'er'
```{r}
...
There must be some problem.. max lifespan can't be a negatuve number. THis is because sometimes scientists mark NAs as -999. Don't record NAs as -999! Leave it blank because that's going to mess up the statistics that you will never find. 
```{r}
x <- mammals[ 11]
colMeans(x)
```
