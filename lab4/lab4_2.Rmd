...
## `filter()`
Unlike `select()`, `filter()` allows us to extract data that meet specific criteria within a variable. Let's say that we are interested only in the fish that occur in lake "AL". We can use `filter()` to extract these observations.  `filter` looks in rows, while `select()` looks in columns. 
```{r}
...
```
```{r}
x <- filter(fish, between(scalelength, 2, 5))
#View(x)
```
**Filter is better than subset**
...
```{r}
filter(relimit_mammals, family == "Bovidae", mass > 450000)
```

```{r}
glimpse(mammals)
```
There's -9990 (placeholders/NAs) so the summary will be wrong. 
...
```{r}
mammals_filtered <- select(mammals, "genus", "mass", "gestation", "newborn")
primate_genera <- filter(mammals_filtered, genus %in% c("Lophocebus", "Erythrocebus", "Macaca"))
primate_genera
```
