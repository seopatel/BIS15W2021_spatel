...
## Load tidyverse
`library()` calls tidyverse, so we are picking up the tidyverse package and putting it into the computer's memory.
```{r message=FALSE, warning=FALSE}
library("tidyverse")
```

## Data Frames
During lab 2, you learned how to work with vectors and data matrices. Today we will organize the vectors into a new type of data structure called a **data frame**. The data frame is the most common way to organize data within R. You can think of a data frame as similar to a spreadsheet. A data frame can store data of many different classes. (while matrices usually store data of only one class).
...
Here we combine our three vectors to create a data frame with the function `data.frame()`. You can see here that the vectors in a data frame are the columns. 
```{r}
...
`str()` is structure. It is commonly used by R programmers.
```{r}
str(hbirds)
```
...
`rbind(where you're adding, what you're adding)`
```{r}
