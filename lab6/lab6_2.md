...
## 3 Torgersen              NA     52
```
***If there is even one NA in the rows, NA will get outputted when running summarize()***
...
What if we are interested in the number of observations (penguins) by species and island? (Do Islands have more than one species of penguin? Torgersen has only one)
...
penguins %>% 
  ...
  # its better to put na.rm in the mean function rather than using filter(!is.na(bill_length_mm))
