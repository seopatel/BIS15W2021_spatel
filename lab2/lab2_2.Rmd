...
## Vectors
Vectors are a common way of organizing data in R.  We create vectors using the `c` command. The `c` stands for concatenate.  An ordered list of numbers or text. 
...
```{r}
harry_potter_matrix <- matrix(box_office, nrow = 8, byrow = T) # byrow is boolean 
...
```
```{r}
Deathly_Hallows_3 <- c(38.0, 96.0)
```
```{r}
all_harry_potter_matrix <- rbind(harry_potter_matrix, Deathly_Hallows_3)
all_harry_potter_matrix
```
...
non_us_earnings #non_us_earnings is a vector
# its better to have all variables of one class in a vector
