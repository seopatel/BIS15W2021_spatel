...
```{r}
#?plotMissing #indicates missing data in the plot 
plotMissing(sug, main="")
# x=chromosome markers, y= mouse ID
plotMissing(sug, main="", reorder=1)
# sometimes scientists only genotype animals with more apparent phenotype. This is important for hw
#?plotMap
plotMap(sug)
...
out.em <- scanone(sug)
out.em2 <- scanone(sug, pheno.col=1:4)#calculates LOD fora ll quanittative phenotypes
```
...
```{r}
summary(out.em, threshold=3) # 1/1000 of the probability you will get this specific phenotype (usually just use 3)
```
...
```{r}
iplotScanone(out.em) # make interactive plot
```
...
```{r}
head(grav$pheno)
iplotCorr(grav$pheno) #correlation between 2 phenotypes
```
## iplotCurves
