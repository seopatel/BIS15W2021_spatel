...
The NA's are represented by -999 in the amniota data set.
...
The Na's are represented as "NA" in the amphibio data set.
...
There are no NA's in the amniota data set. This is because NA's are represented as -999.
...
There are 170691 NA's in amphibio because NA's are represented as "NA" in this data set.
...
```r
amniota_load <- readr::read_csv(file = "data/amniota.csv", na = c("-999", "-30258.711"))
```
