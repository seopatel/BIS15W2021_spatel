...
### Bar Plot: `geom_col()`
Unlike `geom_bar()`, `geom_col()` allows us to specify an x-axis and a y-axis.Use bar for counts and col for specifying the x and y axes
...
homerange %>% 
  filter(class=="mammalia") %>% 
  top_n(-10, log10.mass) %>% #-10 gives top 10 smallest animals
  ggplot(aes(x=common.name, y=log10.mass))+
  geom_col()
```
